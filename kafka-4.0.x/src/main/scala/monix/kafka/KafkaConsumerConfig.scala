...
/** Configuration for Kafka Consumer.
  *
  * For the official documentation on the available configuration options, see
  * [[https://kafka.apache.org/documentation.html#consumerconfigs Consumer Configs]] on `kafka.apache.org`.
  *
  * @param bootstrapServers
  *   is the `bootstrap.servers` setting, a list of host/port pairs to use for establishing the initial connection to
  *   the Kafka cluster.
  *
  * @param fetchMinBytes
  ...
  *
  * @param fetchMaxBytes
  *   is the `fetch.max.bytes` setting, the maximum amount of data the server should return for a fetch request.
  *
  * @param groupId
  *   is the `group.id` setting, a unique string that identifies the consumer group this consumer belongs to.
  *
  * @param heartbeatInterval
  *   is the `heartbeat.interval.ms` setting, the expected time between heartbeats to the consumer coordinator when
  ...
  *   is the `session.timeout.ms` setting, the timeout used to detect failures when using Kafka's group management
  *   facilities.
  *
  * @param sslKeyPassword
  *   is the `ssl.key.password` setting and represents the password of the private key in the key store file. This is
  *   optional for client.
  *
  * @param sslKeyStorePassword
  *   is the `ssl.keystore.password` setting, being the password of the private key in the key store file. This is
  *   optional for client.
  *
  * @param sslKeyStoreLocation
  *   is the `ssl.keystore.location` setting and represents the location of the key store file. This is optional for
  ...
  *   is the `auto.offset.reset` setting, specifying what to do when there is no initial offset in Kafka or if the
  *   current offset does not exist any more on the server (e.g. because that data has been deleted).
  *
  * @param connectionsMaxIdleTime
  ...
  *   is the `enable.auto.commit` setting. If true the consumer's offset will be periodically committed in the
  *   background.
  *
  * @param excludeInternalTopics
  *   is the `exclude.internal.topics` setting. Whether records from internal topics (such as offsets) should be exposed
  *   to the consumer. If set to true the only way to receive records from an internal topic is subscribing to it.
  *
  * @param maxPollRecords
  *   is the `max.poll.records` setting, the maximum number of records returned in a single call to poll().
  *
  * @param receiveBufferInBytes
  *   is the `receive.buffer.bytes` setting, the size of the TCP receive buffer (SO_RCVBUF) to use when reading data.
  *
  * @param requestTimeout
  *   is the `request.timeout.ms` setting, The configuration controls the maximum amount of time the client will wait
  *   for the response of a request. If the response is not received before the timeout elapses the client will resend
  *   the request if necessary or fail the request if retries are exhausted.
  *
  * @param saslKerberosServiceName
  *   is the `sasl.kerberos.service.name` setting, being the Kerberos principal name that Kafka runs as.
  *
  * @param saslMechanism
  *   is the `sasl.mechanism` setting, being the SASL mechanism used for client connections. This may be any mechanism
  ...
  *
  * @param sslProtocol
  *   is the `ssl.protocol` setting, being the SSL protocol used to generate the SSLContext. Default setting is TLS,
  *   which is fine for most cases. Allowed values in recent JVMs are TLS, TLSv1.1 and TLSv1.2. SSL, SSLv2 and SSLv3 may
  *   be supported in older JVMs, but their usage is discouraged due to known security vulnerabilities.
  *
  * @param sslProvider
  *   is the `ssl.provider` setting, being the name of the security provider used for SSL connections. Default value is
  ...
  *
  * @param checkCRCs
  *   is the `check.crcs` setting, specifying to automatically check the CRC32 of the records consumed. This ensures no
  *   on-the-wire or on-disk corruption to the messages occurred. This check adds some overhead, so it may be disabled
  *   in cases seeking extreme performance.
  *
  * @param clientId
  *   is the `client.id` setting, an id string to pass to the server when making requests. The purpose of this is to be
  *   able to track the source of requests beyond just ip/port by allowing a logical application name to be included in
  *   server-side request logging.
  *
  * @param clientRack
  *   is the `client.rack` setting. A rack identifier for this client. This can be any string value which indicates
  *   where this client is physically located. It corresponds with the broker config 'broker.rack'
  *
  * @param fetchMaxWaitTime
  *   is the `fetch.max.wait.ms` setting, the maximum amount of time the server will block before answering the fetch
  *   request if there isn't sufficient data to immediately satisfy the requirement given by fetch.min.bytes.
  *
  * @param metadataMaxAge
  *   is the `metadata.max.age.ms` setting. The period of time in milliseconds after which we force a refresh of
  *   metadata even if we haven't seen any partition leadership changes to proactively discover any new brokers or
  *   partitions.
  *
  * @param metricReporters
  *   is the `metric.reporters` setting. A list of classes to use as metrics reporters. Implementing the
  *   `MetricReporter` interface allows plugging in classes that will be notified of new metric creation. The
  ...
  *   is the `metrics.sample.window.ms` setting. The metrics system maintains a configurable number of samples over a
  *   fixed window size. This configuration controls the size of the window. For example we might maintain two samples
  *   each measured over a 30 second period. When a window expires we erase and overwrite the oldest window.
  *
  * @param reconnectBackoffTime
  *   is the `reconnect.backoff.ms` setting. The amount of time to wait before attempting to reconnect to a given host.
  *   This avoids repeatedly connecting to a host in a tight loop. This backoff applies to all requests sent by the
  *   consumer to the broker.
  *
  * @param reconnectBackoffMaxTime
  *   is the `reconnect.backoff.max.ms` setting. The maximum amount of time to wait when reconnecting to a broker that
  *   has repeatedly failed to connect.
  *
  * @param retryBackoffTime
  *   is the `retry.backoff.ms` setting. The amount of time to wait before attempting to retry a failed request to a
  *   given topic partition. This avoids repeatedly sending requests in a tight loop under some failure scenarios.
  *
  * @param observableCommitType
  *   is the `monix.observable.commit.type` setting. Represents the type of commit to make when the [[enableAutoCommit]]
  *   setting is set to `false`, in which case the observable has to commit on every batch.
  *
  * @param observableCommitOrder
  *   is the `monix.observable.commit.order` setting. Specifies when the commit should happen, like before we receive
  *   the acknowledgement from downstream, or afterwards.
  *
  * @param properties
  *   map of other properties that will be passed to the underlying kafka client. Any properties not explicitly handled
  *   by this object can be set via the map, but in case of a duplicate a value set on the case class will overwrite
  *   value set via properties.
  */
...
object KafkaConsumerConfig {
  ...
  /** Loads the [[KafkaConsumerConfig]] either from a file path or from a resource, if `config.file` or
    * `config.resource` are defined, or otherwise returns the default config.
    *
    ...
    * }}}
    *
    * In case neither of these are specified, then the configuration loaded is the default one, from the `monix-kafka`
    * project, specified in `monix/kafka/default.conf`.
    */
  ...
  /** Loads a [[KafkaConsumerConfig]] from a project resource.
    ...
    *   should be `true` in case you want to fallback to the default values provided by the `monix-kafka` library in
    *   `monix/kafka/default.conf`
    */
  def loadResource(
  ...
  /** Loads the [[KafkaConsumerConfig]] from a parsed `com.typesafe.config.Config` reference.
    *
    * NOTE that this method doesn't assume any path prefix for loading the configuration settings, so it does NOT assume
    * a root path like `kafka`. In case case you need that, you can always do:
    *
    * {{{
