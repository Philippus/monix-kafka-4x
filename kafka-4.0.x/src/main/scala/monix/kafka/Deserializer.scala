...
/** Wraps a Kafka `Deserializer`, provided for convenience, since it can be implicitly fetched from the context.
  ...
  *
  * @param constructor
  *   creates an instance of [[classType]]. This is defaulted with a `Deserializer.Constructor[A]` function that creates
  *   a new instance using an assumed empty constructor. Supplying this parameter allows for manual provision of the
  *   `Deserializer`.
  */
final case class Deserializer[A](
    className: String,
    classType: Class[_ <: KafkaDeserializer[A]],
    constructor: Deserializer.Constructor[A] =
      (d: Deserializer[A]) => d.classType.getDeclaredConstructor().newInstance()
) {
...
object Deserializer {
  ...
  /** Alias for the function that provides an instance of the Kafka `Deserializer`.
    */
