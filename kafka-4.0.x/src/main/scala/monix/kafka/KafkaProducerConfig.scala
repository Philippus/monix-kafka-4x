...
/** The Kafka Producer config.
  ...
  * [[https://kafka.apache.org/documentation.html#producerconfigs Producer Configs]] on `kafka.apache.org`.
  *
  * @param bootstrapServers
  *   is the `bootstrap.servers` setting and represents the list of servers to connect to.
  *
  * @param acks
  *   is the `acks` setting and represents the number of acknowledgments the producer requires the leader to have
  *   received before considering a request complete. See [[monix.kafka.config.Acks Acks]].
  *
  * @param bufferMemoryInBytes
  *   is the `buffer.memory` setting and represents the total bytes of memory the producer can use to buffer records
  *   waiting to be sent to the server.
  *
  * @param compressionType
  *   is the `compression.type` setting and specifies what compression algorithm to apply to all the generated data by
  *   the producer. The default is none (no compression applied).
  *
  * @param retries
  *   is the `retries` setting. A value greater than zero will cause the client to resend any record whose send fails
  ...
  *   is the `batch.size` setting. The producer will attempt to batch records together into fewer requests whenever
  *   multiple records are being sent to the same partition. This setting specifies the maximum number of records to
  *   batch together.
  *
  * @param clientId
  ...
  *
  * @param lingerTime
  *   is the `linger.ms` setting and specifies to buffer records for more efficient batching, up to the maximum batch
  *   size or for the maximum `lingerTime`. If zero, then no buffering will happen, but if different from zero, then
  *   records will be delayed in absence of load.
  *
  * @param maxBlockTime
  *   is the `max.block.ms` setting. The configuration controls how long `KafkaProducer.send()` and
  *   `KafkaProducer.partitionsFor()` will block. These methods can be blocked either because the buffer is full or
  *   metadata unavailable.
  *
  * @param maxRequestSizeInBytes
  *   is the `max.request.size` setting and represents the maximum size of a request in bytes. This is also effectively
  *   a cap on the maximum record size.
  *
  ...
  *   the client will send on a single connection before blocking. If this setting is set to be greater than 1 and there
  *   are failed sends, there is a risk of message re-ordering due to retries (if enabled).
  *
  * @param partitionerClass
  *   is the `partitioner.class` setting and represents a class that implements the
  *   `org.apache.kafka.clients.producer.Partitioner` interface.
  *
  * @param receiveBufferInBytes
  *   is the `receive.buffer.bytes` setting being the size of the TCP receive buffer (SO_RCVBUF) to use when reading
  *   data.
  *
  * @param requestTimeout
  *   is the `request.timeout.ms` setting, a configuration the controls the maximum amount of time the client will wait
  ...
  *
  * @param monixSinkParallelism
  *   is the `monix.producer.sink.parallelism` setting indicating how many requests the [[KafkaProducerSink]] can
  *   execute in parallel.
  *
  * @param properties
...
object KafkaProducerConfig {
  ...
  /** Loads the [[KafkaProducerConfig]] either from a file path or from a resource, if `config.file` or
